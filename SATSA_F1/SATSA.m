function [Best_pos,Best_score,FEs] = SATSA(fhd,dim,nPop,Max_iteration,lb,ub,varargin)
...
for run=1:1
    ...
    while(iter<Max_iteration)
        ...
        for m=1:size(Trees,1)
            ...
            freflec=feval(fhd,xr',varargin{:});
            
            if freflec<ybest
                xe=gamma*xr+(1-gamma)*xc;   % Extension operation by Eq. (12)
                fexp=feval(fhd,xe',varargin{:});
                if fexp<ybest
                    trees(I(m),:)=xe;
                    obj_trees(I(m))=fexp;
                else
                ...
                fcont=feval(fhd,xcont',varargin{:});
                if fcont<ybest
                    trees(I(m),:)=xcont;
                    obj_trees(I(m))=fcont;
                else
                    trees(I(m),:)=1/2*(trees(I(m),:)+bestParams);
                    obj_trees(I(m))=feval(fhd,trees(I(m),:)',varargin{:});
                end
            end
        end
